...
console.log("\n--- Forward Lookup (stroke -> syllable) ---");
console.log(`Lookup KHAUR: ${mapper.lookup(["KHAUR"])}`);     // Expected: kết
console.log(`Lookup TPHA: ${mapper.lookup(["TPHA"])}`);       // Expected: na
console.log(`Lookup SWROFPLT: ${mapper.lookup(["SWROFPLT"])}`); // Expected: xoắn
console.log(`Lookup #KHAUR: ${mapper.lookup(["#KHAUR"])}`);   // Expected: Kết
console.log(`Lookup 25E: ${mapper.lookup(["25E"])}`);       // Expected: Tă (after denumeralize #TAE)
console.log(`Lookup STPHAO: ${mapper.lookup(["STPHAO"])}`);   // Expected: noa (S+TPH+AO -> glide+n+â) Need to check assemble logic

console.log("\n--- Reverse Lookup (syllable -> stroke) ---");
console.log(`Reverse lookup 'kết': ${mapper.reverseLookup("kết")}`);     // Expected: KHAUR (or shortest if multiple)
console.log(`Reverse lookup 'na': ${mapper.reverseLookup("na")}`);       // Expected: TPHA
console.log(`Reverse lookup 'xoắn': ${mapper.reverseLookup("xoắn")}`);   // Expected: SWROFPLT
console.log(`Reverse lookup 'Kết': ${mapper.reverseLookup("Kết")}`);     // Expected: KHAUR (input is case-insensitive)
console.log(`Reverse lookup 'tă': ${mapper.reverseLookup("tă")}`);       // Expected: TAE
console.log(`Reverse lookup 'noa': ${mapper.reverseLookup("noa")}`);     // Expected: STPHAO ? Need verification
console.log(`Reverse lookup 'không': ${mapper.reverseLookup("không")}`); // Should lookup a common word
console.log(`Reverse lookup 'invalid': ${mapper.reverseLookup("invalid")}`); // Expected: undefined

// Test a syllable potentially generated by multiple strokes (if any exist and differ in length)
// This requires knowing the specific outputs of the brute-force generation.
// Example: If both "STROKE1" and "LONGSTROKE2" produce "syllableX", reverseLookup("syllableX") should return "STROKE1".
